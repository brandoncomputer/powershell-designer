	...
	if ($a -eq 'Dpi') {
		if ((get-host).version.major -eq 7) {
			if ((Get-Module -ListAvailable powershell-designer).count -gt 1){
				start-process -filepath pwsh.exe -argumentlist '-ep bypass',"-file $(chr 34)$(path $(Get-Module PowerShell-Designer)[0].path)\DesignerDPI.ps1$(chr 34)"
			}
			else {
				start-process -filepath pwsh.exe -argumentlist '-ep bypass',"-file $(chr 34)$(path $(Get-Module PowerShell-Designer).path)\DesignerDPI.ps1$(chr 34)"
			}
		}
		else {
			if ((Get-Module -ListAvailable powershell-designer).count -gt 1){
				start-process -filepath powershell.exe -argumentlist '-ep bypass',"-file $(chr 34)$(path $(Get-Module PowerShell-Designer)[0].path)\DesignerDPI.ps1$(chr 34)"
			}
			else {
				start-process -filepath powershell.exe -argumentlist '-ep bypass',"-file $(chr 34)$(path $(Get-Module PowerShell-Designer).path)\DesignerDPI.ps1$(chr 34)"
			}
		}
	}
	else{	
